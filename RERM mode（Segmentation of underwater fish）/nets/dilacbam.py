...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.dilaconv2 = nn.Sequential(
            nn.Conv2d(in_planes, in_planes, 3, 1, padding=5, dilation=5, bias=True),
            nn.BatchNorm2d(in_planes, momentum=0.1),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x):
        # print(x.shape)
        x1 = self.dilaconv1(x)
        avg_out =self.shared_MLP(self.avg_pool(x1))
        # print(avg_out.shape)
        # self.fc2(self.relu1(self.fc1(self.avg_pool(x))))
        x2 = self.dilaconv2(x)
        max_out =self.shared_MLP(self.max_pool(x2))
        # print(max_out.shape)
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.dilaconv3 = nn.Sequential(
            nn.Conv2d(320, 32, 3, 1, padding=3, dilation=3, bias=True),
            nn.BatchNorm2d(32, momentum=0.1),
            nn.ReLU(inplace=True),
        )
        #
        # self.dilaconv4 = nn.Sequential(
        #     nn.Conv2d(320, 32, 3, 1, padding=5, dilation=5, bias=True),
        #     nn.BatchNorm2d(32, momentum=0.1),
        #     nn.ReLU(inplace=True),
        # )
